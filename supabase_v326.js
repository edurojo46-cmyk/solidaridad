...
var db = {
    ...
    subscribeToMessages(userId, callback) {
        if (!sbClient) return null;
        // Suscribimos a todos los cambios en la tabla messages. 
        // La seguridad RLS de Supabase se encarga de que solo recibamos lo que nos pertenece.
        // Quitamos los filtros de columna porque en los UPDATE a veces fallan si la columna filtrada no cambiÃ³ (Identity issues).
        return sbClient.channel('chat-realtime-' + userId)
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
                table: 'messages'
            }, function(payload) {
                console.log('[Realtime] Message event:', payload.eventType, payload.new?.id);
                callback(payload.new, payload.eventType, payload);
            })
