...
def patch_index():
    ...
    # 1. Mobile Nav
    mobile_nav_target = """<a href="#" onclick="app.mobileNav('screen-Comedores')"><i class="ri-building-4-line"></i> Comedores</a>"""
    if "screen-anuncios" not in content.split(mobile_nav_target)[-1][:200]:
        mobile_nav_replacement = mobile_nav_target + """\n            <a href="#" onclick="app.mobileNav('screen-anuncios')"><i class="ri-megaphone-line"></i> Anuncios</a>"""
        content = content.replace(mobile_nav_target, mobile_nav_replacement)

    # 2. Desktop Nav
    desktop_nav_target = """<a href="#" onclick="app.navigate('screen-Comedores')">Comedores</a>"""
    if "screen-anuncios" not in content.split(desktop_nav_target)[-1][:200]:
        desktop_nav_replacement = desktop_nav_target + """\n            <a href="#" onclick="app.navigate('screen-anuncios')">Anuncios</a>"""
        content = content.replace(desktop_nav_target, desktop_nav_replacement)

    # 3. Screen Anuncios
    if 'id="screen-anuncios"' not in content:
        screen_target = """    <!-- 2.05 MIS INTENCIONES -->"""
        screen_replacement = """    <!-- 2.01 ANUNCIOS -->
...
def patch_supabase():
    with open('supabase.js', 'r', encoding='utf-8') as f:
        content = f.read()

    if "async getAnuncios(" not in content:
        target = """    // ==================== IGLESIAS COMUNIDAD ===================="""
        code = """    // ==================== ANUNCIOS ====================
    ...
    async uploadAnuncioMedia(file) {
        ...
        const ext = file.name.split('.').pop().lower();
        const bucket = 'chat-media'; // Reusing chat-media bucket
        import random, time
        // Actually this is JS code in a string
...
"""
    ...
    with open('supabase.js', 'w', encoding='utf-8') as f:
        f.write(content)
    print("supabase.js patched")

def patch_app():
    with open('app.js', 'r', encoding='utf-8') as f:
        content = f.read()

    if "openAnuncioModal()" not in content:
        target = """    // Buscar page: filter cards by search text"""
        code = """
    // ==================== ANUNCIOS ====================
    anuncioFile: null,
    ...
    openAnuncioModal() {
        if (!auth.isAuthenticated()) {
            // Permitimos usar el formulario aunque no esté logueado ya que se pidió abierto
            // pero le ponemos anónimo
        }
...
"""
    ...
    nav_target = """        if (screenId === 'screen-map' || screenId === 'screen-Comedores') {"""
    if "this.loadAnuncios();" not in content.split(nav_target)[0][-200:]:
        nav_code = """        if (screenId === 'screen-anuncios') {
            this.loadAnuncios();
        }
"""
        content = content.replace(nav_target, nav_code + nav_target)
