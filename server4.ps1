...
try {
    ...
    # Obtener IP local
    $ip = (Get-NetIPAddress -AddressFamily IPv4 -InterfaceAlias "Wi-Fi", "Ethernet" -ErrorAction SilentlyContinue | Where-Object { $_.IPAddress -like "192.168.*" -or $_.IPAddress -like "10.*" } | Select-Object -ExpandProperty IPAddress -First 1)
    if ($ip) {
        Write-Host "--> ACCESIBLE DESDE TU CELULAR EN: http://$ip`:$port <--" -ForegroundColor Cyan
    } else {
        Write-Host "--> ACCESIBLE DESDE TU CELULAR EN: http://(Tu_IP_Local):$port <--" -ForegroundColor Cyan
    }
} catch {
    Write-Host "Error al iniciar en el puerto $port. Asegúrate de que no esté en uso." -ForegroundColor Red
    exit
}
...
while ($true) {
    try {
        ...
        # Ignorar errores de cliente y continuar escuchando
    }
}
