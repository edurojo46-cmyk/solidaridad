...
function patchIndex() {
    ...
    // 1. Mobile Nav
    if (!indexHtml.includes("app.mobileNav('screen-anuncios')")) {
        const mobileNavTarget = `<a href="#" onclick="app.mobileNav('screen-Comedores')"><i class="ri-building-4-line"></i> Comedores</a>`;
        const mobileNavReplacement = mobileNavTarget + `\n            <a href="#" onclick="app.mobileNav('screen-anuncios')"><i class="ri-megaphone-line"></i> Anuncios</a>`;
        indexHtml = indexHtml.replace(mobileNavTarget, mobileNavReplacement);
    }

    // 2. Desktop Nav
    if (!indexHtml.includes("app.navigate('screen-anuncios')")) {
        const desktopNavTarget = `<a href="#" onclick="app.navigate('screen-Comedores')">Comedores</a>`;
        const desktopNavReplacement = desktopNavTarget + `\n            <a href="#" onclick="app.navigate('screen-anuncios')">Anuncios</a>`;
        indexHtml = indexHtml.replace(desktopNavTarget, desktopNavReplacement);
    }

    // 3. Screen Anuncios
    if (!indexHtml.includes('id="screen-anuncios"')) {
        const screenTarget = `    <!-- 2.05 MIS INTENCIONES -->`;
        const screenReplacement = `    <!-- 2.01 ANUNCIOS -->
    ...
    <!-- 2.05 MIS INTENCIONES -->`;
        indexHtml = indexHtml.replace(screenTarget, screenReplacement);
    }
...
function patchSupabaseJs() {
    let sbHtml = fs.readFileSync('supabase.js', 'utf8');
    if (!sbHtml.includes("async getAnuncios(")) {
        const target = `    // ==================== IGLESIAS COMUNIDAD ====================`;
        const code = `    // ==================== ANUNCIOS ====================
    async createAnuncio(anuncio) {
        if (!sbClient) { saveLocal('anuncios', anuncio); return anuncio; }
        let payload = Object.assign({}, anuncio);
        // let supabase handle id if not valid uuid
    ...
    async uploadAnuncioMedia(file) {
        ...
        const bucket = 'chat-media'; // Reusing chat-media bucket
...
`;
        sbHtml = sbHtml.replace(target, code + target);
        fs.writeFileSync('supabase.js', sbHtml);
        console.log('supabase.js patched');
    }
}

function patchAppJs() {
    let appHtml = fs.readFileSync('app.js', 'utf8');
    if (!appHtml.includes("openAnuncioModal() {")) {
        // Find a safe place to insert methods
        const target = `    // Buscar page: filter cards by search text`;
        const code = `
    // ==================== ANUNCIOS ====================
    anuncioFile: null,
    ...
    async loadAnuncios() {
        ...
        if (anuncios.length === 0) {
            list.innerHTML = '<div class="buscar-empty-state"><i class="ri-megaphone-line"></i><h4>No hay anuncios aún</h4><p>Sé el primero en publicar una actividad solidaria.</p></div>';
            return;
        }
        ...
        anuncios.forEach(anuncio => {
            ...
            card.style.cssText = 'padding: 0; overflow: hidden; border-radius: 16px; margin-bottom: 16px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1);';
...
`;
        appHtml = appHtml.replace(target, code + target);
        
        // Find updateNavVisibility to call loadAnuncios
        const navTarget = `        if (screenId === 'screen-map' || screenId === 'screen-Comedores') {`;
        const navCode = `        if (screenId === 'screen-anuncios') {
            this.loadAnuncios();
        }
`;
        appHtml = appHtml.replace(navTarget, navCode + navTarget);
        
        fs.writeFileSync('app.js', appHtml);
