...
var db = {
    ...
    subscribeToMessages(userId, callback) {
        if (!sbClient) return null;
        var channel = sbClient.channel('chat-realtime-' + userId);
        ...
        return channel
            .on('postgres_changes', {
                ...
                callback(payload.new, payload.eventType, null);
            })
            .on('broadcast', { event: 'reaction' }, function(payload) {
                // El payload de broadcast viene diferente, lo pasamos como 3er argumento
