...
function openChat(partnerId, partnerName) {
    ...
    if (typeof db !== 'undefined' && db.subscribeToMessages) {
        ...
        chatMessagesSubscription = db.subscribeToMessages(currentUser.id, function(newMsg, eventType, broadcastPayload) {
            // 1. Manejar Reacciones instantÃ¡neas (Broadcast)
            if (eventType === 'BROADCAST') {
                var data = broadcastPayload.payload;
            ...
            if (eventType === 'INSERT') {
                // EVITAR DUPLICADOS: Si el mensaje ya existe en el DOM, no hacer nada
